...
// guard) are off-limits to admin and employer accounts: they don't have a profile of their own to
// build, so either lands here only in transit (fresh login, a stale link) and is bounced straight
// to its own console instead of ever rendering the candidate UI. Symmetric with adminGuard/
// employerGuard bouncing a candidate away from /admin/* or /employer/*.
// A 'pending' account (invite-only self-registration awaiting admin approval — see
// upsertOidcUser in auth.service.ts) is bounced to the waiting page ahead of the role check
// below: it has no role-appropriate home yet, since requireAuth blocks it server-side from
...
function redirectAdmin(user: User, router: Router): true | UrlTree {
  if (user.role === 'admin') return router.createUrlTree(['/admin/users']);
  if (user.role === 'employer') return router.createUrlTree(['/employer/requisitions']);
  return true;
}
...
// Same shape as adminGuard, for the employer console (docs/employer-onboarding-spec.md §3's
// features/employer/ area). Real enforcement is requireEmployer server-side
...
export const employerGuard: CanActivateFn = () => {
  ...
  const alreadyResolved = auth.currentUser();
  if (alreadyResolved) {
    return alreadyResolved.role === 'employer' ? true : router.createUrlTree(['/dashboard']);
  }

  return auth.getCurrentUser().pipe(
    map((response) => (response.user.role === 'employer' ? true : router.createUrlTree(['/dashboard']))),
    catchError(() => of(router.createUrlTree(['/login'])))
  );
};
...
// instead of seeing the sign-in form again — or /admin/users for an admin, /employer/requisitions
// for an employer, or /pending for a still-pending account, same split as authGuard above.
...
// already-approved/admin/employer account is sent to its normal home; a signed-out visitor to
// /login.
export const pendingGuard: CanActivateFn = () => {
  const auth = inject(AuthService);
  const router = inject(Router);

  const homeFor = (user: User) => (user.role === 'admin' ? '/admin/users' : user.role === 'employer' ? '/employer/requisitions' : '/dashboard');

  const resolve = (user: User) => (user.status === 'pending' ? true : router.createUrlTree([homeFor(user)]));
