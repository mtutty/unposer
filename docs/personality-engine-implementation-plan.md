...
- `backend/src/models/question-library.ts` — new file, all 25 questions (Q0-Q22 coverage-matrix loadings transcribed from spec §3's table; Q23-Q25 with full text)
...
- **The 22-question library doesn't exist as data anywhere in this repo, this repo's history, or `docs/`** — confirmed via `git grep`/`git log -p` across all commits for question short-names ("Stars on Your Team" etc.). The spec doc's "consolidates three prior workstreams" refers to source material outside this repo that hasn't been supplied. `backend/src/models/question-library.ts` transcribes the §3 coverage-matrix *dimension loadings* for Q0-Q22 (safe — that's the spec's own table data) but leaves `prompt: null` for all 22, with a file-level comment flagging the gap. Q23-Q25 have real verbatim text (spec gives it) and are fully populated. **This blocks Iteration 3** from actually serving Q0-Q22 to a candidate — flagged rather than invented, since spec §1 constraint 2 ("questions must earn the answer") makes this carefully-designed copy, not something to improvise. Whoever owns that original material needs to supply it before Iteration 3 can select real questions beyond Q0/23/24/25.
...
- **Verification:** `npm test` (11 suites / 65 tests, up from 9/38) and `npx tsc --noEmit` both clean. `npm run score:fixture-transcript` run against the real configured model (`anthropic/claude-sonnet-5`) — all 11 hand-built fixtures fired evidence in the expected direction with a non-null score, including the plan's own "I love working alone" → low Work Style example verbatim.
