...
| 7 | Calibration run → unlock scores + culture capture | 🔶 Done with follow-ups | Culture capture + Step 8 gating live; calibration run itself blocked (needs real human raters/transcripts — see notes) |
...
**Scope split, recorded as it happened:** this iteration bundles two genuinely independent things — (a) culture capture and Step 8's tier gate, both pure engineering with no external dependency, and (b) the actual calibration run, which needs real Phase 3 prototype transcripts and a real paid outside rater that don't exist yet in this project. Built (a) in full. Stopped and flagged (b) rather than fabricating ICC numbers or inventing a substitute — doing either would violate the exact thing §9.4 exists to protect ("Nothing here is validated, and the 0-100 precision is theater until it is"). **No numeric scores render anywhere yet** — that half of this iteration's "Done when" line is not met, and can't be until real calibration happens. The calibration console *tool* itself (rating workbench, ICC dashboard) also still isn't built — see `docs/calibration-console-spec.md`, unchanged from Iteration 4.

**Files touched:**
- `backend/src/ai/culture-signal.chain.ts` + `.test.ts` — CVF quadrant inference from Q0/Q15/Q21 answers, zero-to-several signals per candidate (a person's answers can legitimately point to more than one quadrant), hallucinated-exchange-id filtering, same-quadrant de-dup.
- `backend/src/services/culture-signal.service.ts` + `.test.ts` — loads user-role exchanges from Q0/Q15/Q21 threads (whichever exist — not all three required), wholesale-replaces `culture_signal` rows.
- `backend/src/services/profile.service.ts` — `synthesizeProfile` now also regenerates culture signals (fire-and-forget, same posture as the existing evidence re-indexing — never blocks or fails profile generation).
- `backend/src/services/share.service.ts` + new `.test.ts` — `createLink` gated on `progression.tier` reaching Core persona (or later), throwing `TIER_TOO_LOW` with the current tier and which dimensions are still thin, per addendum §7.
- `frontend/src/app/features/onboarding/share/share.component.ts` — proactively disables the "Generate link" form and shows the reason (how many of 11 dimensions are well-evidenced so far) once `progression.tier` is known and below Core persona, reusing the `ProfileService.progression` signal Iteration 5 already built.

**Notes / decisions:**
- **`culture_signal.source_evidence_ids` holds `exchange.id` values, not `dimension_evidence.id` values** — despite the column name (inherited from the spec's own generic naming, shared with the personality-scoring tables). Culture capture works on raw answer text directly; there's no per-culture-signal "evidence extraction" pass the way dimension scoring has one, so the exchange itself is the closest thing to cite. Documented in the chain/service, not silently repurposed.
- **A candidate can have zero, one, or several `culture_signal` rows** — the spec's own data model already shapes this as an array (`culture_signal[]`), and a real candidate's answers plausibly point to more than one quadrant (e.g. Q0 describes a hierarchy-rewarding team, Q15's deal-breaker was specifically about *that* rigidity). Not forced to exactly one row per candidate.
- **Step 8 gate is `progression.tier`, not calibration status** — re-confirmed from addendum §7's own text before implementing: "the Share step's affordance... is enabled only once `progression.tier` reaches Core persona," a tier condition, independent of whether any dimension has cleared calibration. This is why Step 8 gating could ship even though score-unlocking (the other half of this iteration) is fully blocked — they don't actually depend on each other, despite living in the same plan-doc iteration.
- **The calibration-run blocker, stated plainly for whoever picks this up next:** §9.4 needs (1) real transcripts from the Phase 3 prototype validation sessions (15-20 job seekers, per the roadmap) — these don't exist as data in this project yet, this codebase has no user base — and (2) Michael plus at least one paid outside rater (an I/O psychologist) actually rating a sample of that real data through the rating workbench (§5.5, still spec'd-not-built — `docs/calibration-console-spec.md`). Both are outside what an engineering session can produce; there's no defensible substitute (synthetic transcripts can't validate against anything, and a solo rater can't produce the human-vs-human ceiling the whole ship bar is measured against). Whoever runs this: build the console per its spec first (folded into this iteration's scope already per that spec doc's own recommendation), then run it against real collected data.

**Verification:** `npm test` (26 suites / 161 tests, up from 23/148 at Iteration 6), `npx tsc --noEmit`, `npm run build`, and `npm run test:ci` (17/17) all clean. Live against the real dev container and real LLM: opened and answered Q0 for the dev user ("mentored others and built genuine relationships... mattered more than raw output"), ran `CultureSignalService.regenerate` directly — correctly classified as `clan`, with `source_evidence_ids` containing the real `exchange.id` of that answer (verified by direct lookup). `POST /api/share` against the dev user's current (unapproved) profile correctly still 400s `PROFILE_NOT_APPROVED` — confirms the new tier check didn't disturb the existing gate ordering; the tier-gate branch itself is covered by `share.service.test.ts`'s five cases (Sketch rejected with named thin dimensions; Core persona/In depth/Ongoing all allowed; not-approved short-circuits before the tier check ever runs). Test data cleaned up after.
