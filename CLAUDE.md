...
# Test login (no password, not available in production): type any username/email — e.g. devuser
# (role: user) or devadmin (role: admin, bootstraps the first admin) — or a real existing user's
...
Key tables:
- `users` - Auth via OIDC or the test-login bypass
