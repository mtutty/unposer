...
- [ ] Expand test coverage (Jest for backend, Jasmine/Karma for frontend — both wired up and enforced in CI as of `.github/workflows/ci.yml`, no coverage threshold gate, just pass/fail). Backend `services/` (the core business logic) and `ai/` chains are grown alongside features as a matter of course and are reasonably well covered as a result — `services/` is ~16/21 files, `ai/` ~6/12. The real gaps, in priority order: `routes/` (0/13 — `auth.routes.ts` and `webhooks.routes.ts` now covered, see below; the rest are thin validate→service wrappers, lower risk), `websocket/` (0/1), and frontend — only two specs exist (`frontend/src/app/shared/utils/text.ts`, `frontend/src/app/core/flow/flow.service.ts`), no component tests at all. Don't take that last gap as a mandate to backfill component specs wholesale — the UI (admin screens, personality-engine views) is still actively being redesigned, so tests written against it now are likely to be tests rewritten soon; add them as each component's shape actually settles, same as the services/ pattern above.
